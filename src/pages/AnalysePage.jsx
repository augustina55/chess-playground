  ...
  rows.forEach((row, ri) => {
    ...
    for (const ch of row) {
      if ("12345678".includes(ch)) { fi += parseInt(ch); }
        ...
        if (pieceMap[ch]) pos[sq] = pieceMap[ch];
        fi++;
      }
    }
  });
...
function EvalBar({ score, isMate, mateIn, flipped }) {
  const whitePct = useMemo(() => {
    if (isMate) return mateIn > 0 ? 100 : 0;
    const s = Math.max(-1500, Math.min(1500, score ?? 0));
    const pct = 50 + (s / 1500) * 50;
    return flipped ? 100 - pct : pct;
  }, [score, isMate, mateIn, flipped]);

  const label = useMemo(() => {
    if (score === undefined || score === null) return "0.0";
    if (isMate) return `M${Math.abs(mateIn)}`;
    return (Math.abs(score) / 100).toFixed(1);
  }, [score, isMate, mateIn]);

  const isWhiteAdv = (score ?? 0) >= 0;

  return (
    <div className="w-5 flex flex-col rounded-lg overflow-hidden border border-gray-300 self-stretch relative shrink-0">
      <div className="bg-gray-800 transition-all duration-500" style={{ height: `${100 - whitePct}%` }} />
      <div className="bg-gray-100 flex-1 transition-all duration-500" style={{ height: `${whitePct}%` }} />
      <span className={cn(
        "absolute text-[8px] font-black w-full text-center leading-none",
        isWhiteAdv ? "bottom-1 text-gray-800" : "top-1 text-gray-100"
      )}>
        {label}
      </span>
    </div>
...
export default function AnalysePage() {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(([entry]) => {
      ...
      setBoardSize(Math.floor(Math.min(width, height)) - 4);
    });
    ro.observe(el);
  ...
  function handlePieceDrop(src, tgt) {
    for (const fenToTry of [currentFen, flipTurn(currentFen)]) {
      ...
      let move = null;
      try { move = g.move({ from: src, to: tgt, promotion: "q" }); } catch {}
  ...
  const engineArrows = useMemo(() => {
    ...
    return [[bm.slice(0, 2), bm.slice(2, 4), "rgba(21,130,21,0.85)"]];
    ...
    for (let rank = 8; rank >= 1; rank--) {
      ...
      for (let fi = 0; fi < 8; fi++) {
        ...
        if (p) {
          ...
          const ch = { wP:"P",wN:"N",wB:"B",wR:"R",wQ:"Q",wK:"K",bP:"p",bN:"n",bB:"b",bR:"r",bQ:"q",bK:"k" }[p];
          row += (ch || "?");
        } else { empty++; }
      }
      if (empty) row += empty;
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-[#f6f8fc]">
      {/* ── Toolbar ── */}
            ...
            {editMode ? (
              <Chessboard
                ...
                onPieceDrop={(src, tgt, piece) => {
                  setEditPos(prev => { const n = { ...prev }; delete n[src]; n[tgt] = piece; return n; });
                  return true;
                }}
                onSquareClick={sq => {
                  setEditPos(prev => {
                    const n = { ...prev };
                    if (eraserOn) delete n[sq];
                    else n[sq] = selectedPiece;
                    return n;
                  });
                }}
              />
          ...
          <div className="shrink-0 border-b border-gray-100 p-3 space-y-2">
            ...
            {engineOn && info && (
              <div className="bg-gray-50 rounded-xl p-2.5 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] text-gray-400">Eval</span>
                  <span className={cn(
                    "text-[14px] font-black tabular-nums",
                    (info.score ?? 0) > 50 ? "text-gray-900" : (info.score ?? 0) < -50 ? "text-gray-500" : "text-gray-700"
                  )}>
                    {scoreDisplay ?? "—"}
                  </span>
                </div>
                  ...
                  {["w", "b"].map(side => (
                    <button
                      ...
                      className={cn(
                        ...
                        editTurn === side
                          ? side === "w" ? "border-gray-700 bg-gray-100 text-gray-900" : "border-gray-800 bg-gray-800 text-white"
